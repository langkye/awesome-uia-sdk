...
class AccessTokenRequest : AbstractUiaRequest(), IUiaRequest {
    ...
    override fun url(property: IUiaProperty): String {
        ...
        return try {
            (varProperty.domain + varProperty.accessTokenUrl
                + "?grant_type=authorization_code"
                + "&code=${body}"
                + "&client_id=${varProperty.clientId}"
                + "&client_secret=${varProperty.clientSecret}"
                + "&redirect_uri=${URLEncoder.encode(varProperty.redirectUri, "UTF-8")}"
            )
        } catch (e: Exception) {
