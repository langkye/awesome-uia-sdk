package cn.lnkdoc.sdk.uia.instance.gitee.client
 ...
 * @since 1.0.0.RELEASE
 */
class GiteeUiaClient private constructor() : IUiaClient {
    private val client = OkHttpClient().newBuilder().build()
    private lateinit var property: GiteeProperty
    ...
    override fun <RESP> execute(request: IUiaRequest): UiaResponse<RESP> {
        return try {
            ...
            val data = try {
                converter.convertResponse<RESP, Tuple2<String, GiteeProperty?>>(Tuple.of(string, property))
    ...
    private fun sendRequest(request: IUiaRequest): String {
        ...
        var string = ""
        val body =
            converter.convertRequest<RequestBody, Tuple2<IUiaRequest?, GiteeProperty?>>(Tuple.of(request, property))
        val headers = converter.buildHeaders<Headers, Tuple2<IUiaRequest?, GiteeProperty?>>(Tuple.of(request, property))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(GiteeUiaClient::class.java)
         ...
         */
        @JvmStatic
        fun getInstance(property: GiteeProperty): GiteeUiaClient {
            val client = GiteeUiaClient()
        ...
        private fun checkMustRequired(property: GiteeProperty) {
            ...
            required(property.userInfoUrl, "the userInfoUrl configuration is required")
            required(property.refreshTokenUrl, "the refreshTokenUrl configuration is required")
        }
    }
}
