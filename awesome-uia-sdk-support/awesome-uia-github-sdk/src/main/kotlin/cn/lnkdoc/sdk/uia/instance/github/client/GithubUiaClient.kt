package cn.lnkdoc.sdk.uia.instance.github.client
 ...
 * @since 1.0.0.RELEASE
 */
class GithubUiaClient private constructor() : IUiaClient {
    private val client = OkHttpClient().newBuilder().build()
    private lateinit var property: GithubProperty
    ...
    override fun <RESP> execute(request: IUiaRequest): UiaResponse<RESP> {
        return try {
            ...
            val data = try {
                converter.convertResponse<RESP, Tuple2<String, GithubProperty?>>(Tuple.of(string, property))
    ...
    private fun sendRequest(request: IUiaRequest): String {
        ...
        var string = ""
        val body =
            converter.convertRequest<RequestBody, Tuple2<IUiaRequest?, GithubProperty?>>(Tuple.of(request, property))
        val headers = converter.buildHeaders<Headers, Tuple2<IUiaRequest?, GithubProperty?>>(Tuple.of(request, property))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(GithubUiaClient::class.java)
         ...
         */
        @JvmStatic
        fun getInstance(property: GithubProperty): GithubUiaClient {
            val client = GithubUiaClient()
