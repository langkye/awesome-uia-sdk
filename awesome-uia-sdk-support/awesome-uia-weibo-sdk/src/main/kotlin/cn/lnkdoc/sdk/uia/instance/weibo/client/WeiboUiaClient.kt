package cn.lnkdoc.sdk.uia.instance.weibo.client
 ...
 * @since 1.0.0.RELEASE
 */
class WeiboUiaClient private constructor() : IUiaClient {
    private val client = OkHttpClient().newBuilder().build()
    private lateinit var property: WeiboProperty
    ...
    override fun <RESP> execute(request: IUiaRequest): UiaResponse<RESP> {
        return try {
            ...
            val data = try {
                converter.convertResponse<RESP, Tuple2<String, WeiboProperty?>>(Tuple.of(string, property))
    ...
    private fun sendRequest(request: IUiaRequest): String {
        ...
        var string = ""
        val body =
            converter.convertRequest<RequestBody, Tuple2<IUiaRequest?, WeiboProperty?>>(Tuple.of(request, property))
        val headers = converter.buildHeaders<Headers, Tuple2<IUiaRequest?, WeiboProperty?>>(Tuple.of(request, property))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(WeiboUiaClient::class.java)
         ...
         */
        @JvmStatic
        fun getInstance(property: WeiboProperty): WeiboUiaClient {
            val client = WeiboUiaClient()
            client.property = property
            checkMustRequired(property)
            return client
        }

        private fun checkMustRequired(property: WeiboProperty) {
