package cn.lnkdoc.sdk.uia.instance.gitlab.client
 ...
 * @since 1.0.0.RELEASE
 */
class GitlabUiaClient private constructor() : IUiaClient {
    private val client = OkHttpClient().newBuilder().build()
    private lateinit var property: GitlabProperty
    ...
    override fun <RESP> execute(request: IUiaRequest): UiaResponse<RESP> {
        return try {
            ...
            val data = try {
                converter.convertResponse<RESP, Tuple2<String, GitlabProperty?>>(Tuple.of(string, property))
    ...
    private fun sendRequest(request: IUiaRequest): String {
        ...
        var string = ""
        val body =
            converter.convertRequest<RequestBody, Tuple2<IUiaRequest?, GitlabProperty?>>(Tuple.of(request, property))
        val headers = converter.buildHeaders<Headers, Tuple2<IUiaRequest?, GitlabProperty?>>(Tuple.of(request, property))
    ...
    companion object {
        private val log = LoggerFactory.getLogger(GitlabUiaClient::class.java)
         ...
         */
        @JvmStatic
        fun getInstance(property: GitlabProperty): GitlabUiaClient {
            val client = GitlabUiaClient()
            client.property = property
            checkMustRequired(property)
            return client
        }

        private fun checkMustRequired(property: GitlabProperty) {
            ...
            required(property.redirectUri, "the redirectUri configuration is required")
        }
    }
}
